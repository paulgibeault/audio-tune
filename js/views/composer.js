...
export class ComposerView {
  constructor(root, { prefs, wakeScope }) {
    this.root = root;
    this.prefs = prefs;
  ...
  renderEditor() {
    ...
    M.replaceChildren();
    if (!this.cue) {
      M.append(el('section', { class: 'card' }, el('h2', { class: 'card-h' }, 'Build a sound'), el('p', { class: 'card-sub' }, 'Stack a few gestures — a strike a few milliseconds before a body is a knock. Start with New sound.')));
      return;
    }
    const c = this.cue;
    ...
    addSel.addEventListener('change', () => {
      if (!addSel.value || c.layers.length >= 12) return;
    ...
    M.append(el('section', { class: 'card' }, el('div', { class: 'row' }, addSel, el('span', { class: 'ctl-hint' }, `${c.layers.length} of 12 layers`))));
    // code
    M.append(el('section', { class: 'card' }, el('h3', { class: 'card-h' }, 'Pack code', panelHelp(PANELS.code.title, PANELS.code.body)), code));
    this.refreshCode();
    // room
  ...
  renderLayer(c, L, i) {
    ...
    card.append(
      el('h3', { class: 'card-h' }, el('span', { class: 'layer-no' }, `${i + 1}`), elSel, panelHelp(L.el, def.note),
        el('span', { class: 'layer-tools' },
          ...
          el('button', { class: 'mini', type: 'button', 'aria-label': 'duplicate', disabled: c.layers.length >= 12, onclick: () => { c.layers.splice(i + 1, 0, JSON.parse(JSON.stringify(L))); this.changed(); this.renderEditor(); } }, '⧉'),
    ...
    card.append(top);
    // the element's own knobs, through the shared builder
    const st = { ...(L.params || {}) };
    ...
    card.append(elementControls(L.el, st, sync));
    return card;
  }
